...
def solveur_profondeur(grille, chemin, visite, map, regle, affiche, sauvegarde):
    ...
    for next_pos in mvt_possible:
        if (tuple(next_pos), tuple(vitesse)) not in visite:
            ...
            elif jeu.victoire(chemin + [next_pos], grille):
                save.sauvegarde_partie(chemin + [next_pos], map, sauvegarde, regle)
...
def solveur_largeur(grille, visite, map, regle, affiche, sauvegarde):
    ...
    pion, pos_depart=grillage.miseenplacepion(grille)
        ...
        for next_pos in mvt_possible_depart:
            if (tuple(next_pos), tuple(vitesse_depart)) not in visite_depart:
                if tuple(next_pos) in visite_arrivee:
                    save.sauvegarde_partie(chemin_depart + chemin_arrivee[::-1], map, sauvegarde, regle)
        ...
        for next_pos in mvt_possible_arrivee:
            if (tuple(next_pos), tuple(vitesse_arrivee)) not in visite_arrivee:
                if tuple(next_pos) in visite_depart:
                    save.sauvegarde_partie(chemin_arrivee + chemin_depart[::-1], map, sauvegarde, regle)
...
def solveur(choix, map, regle, affiche, sauvegarde):
    grille=conversion.conversion_txt(map)
    pion, pos_depart = grillage.miseenplacepion(grille)
...
solveur("Random", "map1.txt", "souple", [False, True], "9")